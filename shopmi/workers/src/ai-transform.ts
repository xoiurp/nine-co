...
function generalizeBrandText(title: string, description?: string): string {
  ...
  const categories: Record<string, RegExp[]> = {
    'Smartphone': [
      /\b(celulares?|smartphones?|telefones?|phones?)\b/i, // Aceita plural
      /\b(note|mi|redmi|poco)\s*[a-z]?\d+/i, // Aceita A5, Note 10, etc
      /\b(4g|5g)\b/i,
      /\b(android|ios)\b/i,
    ],
    'Fone de ouvido': [
      /\b(fones?|headphones?|earphones?|earbuds?|airdots)\b/i, // Aceita plural
      /\b(wireless|bluetooth|true wireless)\b/i,
    ],
    'Carregador': [
      /\b(carregadores?|chargers?|adapters?|fontes?)\b/i, // Aceita plural
      /\b(\d+w)\b/i,
    ],
    'Smartwatch': [
      /\b(smartwatchs?|watchs?|relógios?)\b/i, // Aceita plural
      /\b(band|fit)\b/i,
    ],
    'Tablet': [
      /\b(tablets?|pads?)\b/i, // Aceita plural
    ],
    'Notebook': [
      /\b(notebooks?|laptops?)\b/i, // Aceita plural
    ],
    'Acessório': [
      /\b(capas?|cases?|películas?|protetores?)\b/i, // Aceita plural
    ],
  };
  ...
  if (specs.length > 0) {
    result += ' ' + specs.join(' ');
  } else if (genericTitle && genericTitle.length > 0) {
    // Se não encontrou specs mas sobrou texto, usar o que sobrou
    result += ' ' + genericTitle;
  }
